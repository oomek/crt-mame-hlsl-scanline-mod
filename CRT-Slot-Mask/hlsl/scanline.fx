...
float4 ps_main(PS_INPUT Input) : COLOR
{
	...
	float BrightnessOffset = ScanlineBrightOffset;
	float BrightnessScale = ScanlineBrightScale * 2.0f;

	float ColorBrightness = (BaseColor.r + BaseColor.g + BaseColor.b) / 3.0f;
	...
	float ScanlineSine = sin(ScanlineCoord);
	float ScanlineAmount = ScanlineSine * ScanlineSine;
	float ScanlineBrightness = ScanlineAmount * BrightnessScale + BrightnessOffset * BrightnessScale;
	BaseColor.rgb = normalize(BaseColor.rgb) * ColorBrightness * 2.0f * ScanlineBrightness;
	BaseColor = lerp(BaseColor, SourceColor, ColorBrightness);
	BaseColor = lerp(SourceColor, BaseColor, ScanlineAlpha);
